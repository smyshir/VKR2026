...
from ..services import (
    weighted_least_squares_intersection,
    area_center_and_radius,
    pairwise_intersections,
    filter_anomalous_points,
    convex_hull,
)
...
def stream_run(payload: StreamRunRequest, db: Session = Depends(get_db)):
    ...
    # Область формируется по попарным пересечениям пеленгов с удалением аномалий, затем строится выпуклая оболочка.
    ...
    return StreamRunOut(
        ...
        center_y=center_y,
        radius=radius,
        region_polygon=[[x, y] for x, y in region],
        suggested_grid_segments=payload.grid_segments,
        recommended_weapon=_pick_weapon(db, radius),
